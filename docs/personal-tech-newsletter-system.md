...
```text
官方源 / 一手源 -> RSS / Atom 抓取 -> 去重入库 -> 候选文章查询 -> Markdown 简报草稿 -> AI 摘要/润色 -> 邮件推送 -> 归档
```
...
- 通过内部 REST API 手动触发 RSS 入库、查询 RSS 运行记录、生成 Markdown 简报草稿和 AI 摘要，并通过 OpenAPI / Swagger UI 管理接口文档。

邮件推送、归档表和运行告警仍属于后续阶段。
...
- `brief`：基于候选文章生成 Markdown 简报草稿，并编排 AI 摘要生成。
...
2. 使用真实 AI Provider 验证摘要 prompt、输出结构和超时配置。
3. 实现邮件发送和简报归档。
4. 扩展端到端运行记录，沉淀摘要状态、发送状态和归档状态。
