...
1. 增强 RSS / Atom 内容提取策略，除 `description` / `summary` 外兜底读取 `content:encoded` 和 Atom `content`，并用 jsoup 清洗 HTML，避免 Spring Blog 等源丢失有效摘要。
2. 建立新增 RSS 源质量审计清单，检查字段完整性、内容所在字段、是否全文或短摘要、是否与已有源重复，再决定是否纳入默认源。
3. 给关键链路补充克制的运行日志，覆盖 RSS 入库、AI Provider 调用、简报归档和邮件发送，便于试运行排查但不输出密钥、正文全文或授权头。
4. 增加文章内容增强模块，针对 RSS 摘要过短或缺失的文章抓取原文正文、清洗 HTML、截断入库，并让 AI 摘要优先消费增强内容。
