...
- 真实 Provider 试运行已验证 Chat Completions 输出结构和归档成功链路；一次不落库请求曾在约 30 秒命中读取超时，试运行阶段建议把 `signal-brief.ai-summary.read-timeout` 调整为 `60s`。
- 根据真实输出继续微调 prompt，重点避免在草稿摘要缺失时过度推断。
