...
RSS 入库链路已经具备配置源读取、RSS / Atom 解析、去重入库、定时触发、手动触发和简报候选查询。为了支撑后续 AI 摘要、Markdown 简报、邮件推送和归档，系统需要一组可维护的真实 RSS 源，并且在外部网络波动、站点限流或服务端错误时能给出清晰、可诊断的失败信息。

本阶段只增强“外部 feed 抓取”这条边界，不引入源管理表、任务运行记录、告警系统、代理池、全文爬虫或 Web 管理界面。
...
配置通过 `FeedHttpProperties` 绑定和校验。新增或调整环境变量时，需要同步 `.env.example`、README 和相关 record。
...
本阶段仍只在 `FeedIngestionResult` 中累计失败源数量。源级失败明细持久化、任务运行记录和告警属于后续设计。
...
- 增加任务运行记录表，保存开始时间、结束时间、触发方式、源级统计和失败原因。
- 为连续失败源增加告警策略。
