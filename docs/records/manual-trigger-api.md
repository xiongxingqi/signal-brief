...
当前已经覆盖：RSS 入库手动触发、RSS 入库运行记录查询、Markdown 草稿手动生成、默认关闭的内部 API、默认关闭的 OpenAPI / Swagger UI、基础错误响应。

当前暂不覆盖：认证授权、AI 摘要、邮件发送、简报归档、CLI 入口、运行记录告警/清理和多实例运行锁。
...
- 接入 AI 摘要、邮件发送和归档后，评估是否扩展端到端运行记录。
